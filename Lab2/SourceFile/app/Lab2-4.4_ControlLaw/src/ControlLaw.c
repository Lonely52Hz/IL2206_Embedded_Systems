...
void ControlTask(void* pdata)
{
  ...
  while(1)
  {
    ...
    if (cruise_activated == on) {
        if (28 <= *current_velocity && *current_velocity < 29) {
            throttle = 15;
        }
        else if (*current_velocity >= 29) {
            throttle = 5;
        }
        else if (21 < *current_velocity && *current_velocity <= 22) {
            throttle = 50;
        }
        else if (*current_velocity <= 21) {
            throttle = 60;
        }
        else {
            throttle = 30;
        }
        show_target_velocity(25);
    }
    else {
        if (enginestate == on) {
            if (*gas_pedal == on) {
